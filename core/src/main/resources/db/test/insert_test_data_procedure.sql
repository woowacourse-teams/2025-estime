...
BEGIN
    ...
    -- 방(Room) 생성 메인 루프
    WHILE v_room_counter <= v_total_rooms DO

            -- 시나리오 결정
            IF v_room_counter <= (v_total_rooms * 0.15) THEN
                SET v_scenario = '활발한 방'; SET v_num_participants = FLOOR(8 + RAND() * 3); SET v_num_dates = FLOOR(5 + RAND() * 3); SET v_num_times = FLOOR(32 + RAND() * 17); SET v_vote_probability = 0.7;
            ELSEIF v_room_counter <= (v_total_rooms * 0.55) THEN
                SET v_scenario = '일반적인 방'; SET v_num_participants = FLOOR(5 + RAND() * 4); SET v_num_dates = FLOOR(3 + RAND() * 3); SET v_num_times = FLOOR(16 + RAND() * 17); SET v_vote_probability = 0.4;
            ELSEIF v_room_counter <= (v_total_rooms * 0.90) THEN
                SET v_scenario = '저조한 방'; SET v_num_participants = FLOOR(2 + RAND() * 4); SET v_num_dates = FLOOR(2 + RAND() * 4); SET v_num_times = FLOOR(8 + RAND() * 17); SET v_vote_probability = 0.15;
            ELSE
                SET v_scenario = '막 생성된 방'; SET v_num_participants = FLOOR(3 + RAND() * 4); SET v_num_dates = FLOOR(4 + RAND() * 4); SET v_num_times = FLOOR(16 + RAND() * 17); SET v_vote_probability = 0.0;
            END IF;
            ...
            VALUES (
                       DATE_ADD('2027-01-01', INTERVAL FLOOR(RAND() * 30) DAY),
                       v_session,
            ...
            WHILE v_participant_counter <= v_num_participants DO
            ...
            CREATE TEMPORARY TABLE temp_dates (gen_date DATE);
            CREATE TEMPORARY TABLE temp_times (gen_time TIME);
            SET v_start_date = DATE_ADD('2026-01-01', INTERVAL FLOOR(RAND() * 10) DAY);
            INSERT INTO temp_dates (gen_date) WITH RECURSIVE DateSeries AS (SELECT 0 AS offset UNION ALL SELECT offset + 1 FROM DateSeries WHERE offset + 1 < v_num_dates) SELECT DATE_ADD(v_start_date, INTERVAL offset DAY) FROM DateSeries;
            INSERT INTO temp_times (gen_time) WITH RECURSIVE TimeSeries AS (SELECT 0 AS offset UNION ALL SELECT offset + 1 FROM TimeSeries WHERE offset + 1 < v_num_times) SELECT SEC_TO_TIME(offset * 1800) FROM TimeSeries;
            ...
            IF v_vote_probability > 0 THEN
                ...
                WHERE p.room_id = v_room_id AND RAND() < v_vote_probability;
            END IF;
...
UNION ALL
SELECT 'participant' AS table_name, COUNT(*) AS row_count FROM participant
UNION ALL
SELECT 'room_available_date_slot' AS table_name, COUNT(*) AS row_count FROM room_available_date_slot
UNION ALL
SELECT 'room_available_time_slot' AS table_name, COUNT(*) AS row_count FROM room_available_time_slot
UNION ALL
SELECT 'vote' AS table_name, COUNT(*) AS row_count FROM vote
UNION ALL
SELECT 'platform' AS table_name, COUNT(*) AS row_count FROM platform
