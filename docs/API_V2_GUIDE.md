...
- DB 스토리지 절감: 8 bytes → 3 bytes (20 bits 데이터)
- 네트워크 전송 효율 향상 (TCP slow start)
- V1과 병렬 운영 가능
...
HTTP 응답이 **14.6 KB**를 초과하면 TCP slow start로 인해 추가 왕복(round trip)이 발생합니다.

**임계값 근거:**

- Initial Congestion Window: 10 segments (RFC 6928)
- MSS: 1,460 bytes (MTU 1,500 - TCP/IP 헤더)
- 첫 왕복 최대 전송: 10 × 1,460 = **14,600 bytes**

**실제 시나리오 (참가자 5명, 투표 슬롯 125개):**

| 버전                 | 응답 크기      | Round Trips |
|--------------------|------------|-------------|
| V1 (LocalDateTime) | XX KB      | 2회 이상       |
| V2 (압축 슬롯)         | ~14 KB     | 1회          |
| **절감률**            | **XX% 감소** |             |

압축 인코딩으로 단일 왕복 안에 응답을 전송하여 지연을 최소화합니다.

#### 데이터 표현

슬롯 코드는 정수(integer)로 전송합니다.
