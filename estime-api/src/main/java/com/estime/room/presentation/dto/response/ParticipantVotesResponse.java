...
public record ParticipantVotesResponse(
    ...
    public static ParticipantVotesResponse from(final Votes votes, final String participantName) {
        return new ParticipantVotesResponse(participantName,
                votes.getElements()
                        .stream().map(vote -> vote.getId().getDateTimeSlot().getStartAt())
                        .sorted()
                        .toList()
        );
    }
