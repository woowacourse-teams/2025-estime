...
const useLocalTimeSelectionThrottled = ({ initialSelectedTimes }: UseLocalTimeSelectionOptions) => {
  ...
  // 배치(rAF) - handleDragMove throttling용
  const dragMoveAnimationFrameId = useRef<number | null>(null);
  ...
  const handleDragMove = useCallback((event: React.PointerEvent) => {
    ...
    // expensive한 collision detection을 rAF로 throttling
    if (dragMoveAnimationFrameId.current != null) return;

    dragMoveAnimationFrameId.current = requestAnimationFrame(() => {
      dragMoveAnimationFrameId.current = null;
  ...
  const resetDragState = useCallback(() => {
    ...
    // cleanup pending rAF
    if (dragMoveAnimationFrameId.current != null) {
      cancelAnimationFrame(dragMoveAnimationFrameId.current);
      dragMoveAnimationFrameId.current = null;
    }
  }, []);
...
export default useLocalTimeSelectionThrottled;
